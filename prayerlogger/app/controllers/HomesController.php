...
class HomesController extends \BaseController {
	...
	public function index()
	{
		...
		$total__asar_days = DB::table('prayerlogs')->where('user_id', '=', $current_user_id)
		->where('prayer_name', 'Asr')
		->count();

		$total__maghrib_days = DB::table('prayerlogs')->where('user_id', '=', $current_user_id)
		->where('prayer_name', 'Mughrab')
		->count();

		$total__ishaa_days = DB::table('prayerlogs')->where('user_id', '=', $current_user_id)
...
//dd($unoffered_ishaa);
		...
		$data = array(
			...
			$unlogged_maghrib,
			$unoffered_maghrib,
			$logged_regular_maghrib,
			$logged_qaza_maghrib,
			$logged_qasar_maghrib,
